...
* 输入的历史命令默认保存在`~/.local/state/hss/history`文件中（遵循XDG基础目录规范）。如果无法创建XDG目录，则回退到`~/.hss_history`以保持向后兼容性。
* 远程命令和文件路径补全，数据信息来自于第一个host连接
* 进行文件路径补全时，需要当前单词的首字符是`/`, `~`, `.`才可提示补全

### 环境变量

* `HSS_HISTORY_FILE` - 自定义命令历史文件路径。优先于XDG路径和传统位置。
* `XDG_STATE_HOME` - 状态文件的基础目录（默认为`~/.local/state`）。历史文件将保存在`$XDG_STATE_HOME/hss/history`。

### 每个主机的彩色输出

会话中的每个主机都会从循环调色板（红、绿、黄、蓝、品红、青）中分配一个唯一的颜色，以便在多主机会话中更容易直观地区分来自不同服务器的输出。
